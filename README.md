...
macOS 产物在 GitHub Actions 配置了 Apple 签名凭据时会自动签名并公证；未配置时会退回 ad-hoc 签名，首次打开仍可能需要在“系统设置 -> 隐私与安全性”中手动允许。
...
- 如需让 macOS 下载包避免 Gatekeeper 的“已损坏/无法验证开发者”提示，需要在 GitHub Actions Secrets 中配置 `MACOS_CERTIFICATE_P12`、`MACOS_CERTIFICATE_PASSWORD`、`MACOS_KEYCHAIN_PASSWORD`、`APPLE_ID`、`APPLE_APP_PASSWORD`、`APPLE_TEAM_ID`
